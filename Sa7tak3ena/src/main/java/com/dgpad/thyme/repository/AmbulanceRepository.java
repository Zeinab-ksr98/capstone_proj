...
public interface AmbulanceRepository extends JpaRepository<Ambulance, UUID> {
    ...
    @Query("SELECT DISTINCT a FROM Ambulance a JOIN a.ambulanceCars ac WHERE a.enabled = true and a.Agency = :agency AND ac.type = :type AND ac.status = com.dgpad.thyme.model.enums.AmbulanceStatus.Avilable")
